...
from px_device_identity.errors import CMAuthenticationFailed, CMServerError
...
class CM:
    ...
    def _post_registration(self, registration: 'DeviceRegistrationProperties'):
        ...
        try:
            log.info("=> Posting registration to {}".format(
                self.api_register_url))
            result = post(self.api_register_url,
                          json=self._registration_content(registration))
            ...
            log.error('Connection to {} failed. Please verify the address.'.format(
                self.api_register_url))
            raise
    ...
    def check_registration_result(self):
        ...
        try:
            ...
            log.error(
                "Something went wrong checking for the registration result: {}".format(err))
            raise err
    ...
    def _check_registration_result_loop(self):
        ...
        for i in range(limit):
            ...
            if result is False:
                raise Exception(
                    'Could not complete registration. Timeout exceeded.')
            ...
            if status_code == 200:
                ...
                if status == 'error':
                    raise Exception(
                        'Something went wrong: Received an error from the server.')
            else:
                log.error(
                    "Request failed with status code {}".format(status_code))
                if status_code == 404:
                    log.error(
                        "Cannot find pending registration. Did you register already?")
                if status_code == 500:
                    log.error(
                        "Did not receive expected response from the server. Please contact the administrator.")
                    raise Exception(
                        'Did not receive expected response from the server. Please contact the administrator.')
    ...
    def register_device(self, registration: 'DeviceRegistrationProperties'):
        ...
        result = self._check_registration_result_loop()
        if result == None:
            raise ValueError('Unexpected registration result.')
        device_id, client_id = result
        return device_id, client_id
    ...
    def request_access_token(self, device_jwt: str):
        ...
        if response.status_code == 200:
            ...
            current_time = get_unix_time_in_seconds()
            expires_at = current_time + response_content['expires_in']  # 600s
            return {
                'access_token': response_content['access_token'],
                'expires_at': expires_at
            }
        if response.status_code == 401:
            raise CMAuthenticationFailed('Failed to get access token (401).')
