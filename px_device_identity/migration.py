...
def first_migration_key_dir(key_dir=KEY_DIR()):
    if os.path.isdir(KEY_DIR_LEGACY()):
        ...
        log.warning(
            'Found old key dir at {}. From v0.7.0 the key dir resides at {}'.format(KEY_DIR_LEGACY(), key_dir)
        )
        if os.path.isdir(key_dir):
            log.warning(
                'Config has already been migrated. Delete {} whenver you are ready.'.format(KEY_DIR_LEGACY())
            )
        else:
            os.makedirs(key_dir)
            ...
            for file_name in files:
                ...
                log.warning(
                    'All files have been copied. Delete {} whenver you are ready.'.format(KEY_DIR_LEGACY())
                )
        # From 0.9.0
